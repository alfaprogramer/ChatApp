...
const ChatRoom = () => {
  ...
  const sendMessage = async (senderId, receiverId) => {
    try {
      await axios.post('http://192.168.18.3:8000/sendMessage', {
        senderId,
        receiverId,
        message,
      });
  ...
  const fetchMessages = async () => {
    try {
      ...
      const response = await axios.get('http://192.168.18.3:8000/messages', {
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: 'white' }}>
      ...
      >
        {messages?.map((item, index) => (
          <Pressable
            key={item._id || index} // Use unique ID if available, otherwise fallback to index
            style={[
